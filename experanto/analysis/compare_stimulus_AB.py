...
AB_list = ["AB_null35", "AB_null1035", "AB_null3035", "AB_null9035", "AB_1sec", "AB_3sec", "AB_9sec"]
...
for ab_name in AB_list:
    datasets_mid[ab_name] = PickledDataStore(
        load=True,
        parameters=ParameterSet({"root_directory": f"SelfSustainedPushPull_test:{ab_name}_____", "store_stimuli": False}),
        replace=False
    )
...
for name in AB_list:
    ...
    print(movie_A_name)
    A_from_AB = [seg for seg, stim_dict in parsed if stim_dict['movie_name'] == movie_A_name]
...
for i, AA in enumerate(AA_list):
    for j, AB in enumerate(AB_list):
        ...
        corr = plot_corr(values_A, values_A1, f"results/AB_time_comp_correct/response_comparison_{AA}_{AB}.png")
        data[AB][AA] = corr
...
ab_labels = [x for x in reversed(AB_list)] # ["AB1sec", "AB3sec", "AB5sec", "AB7sec", "AB9sec", "AB15sec", "AB25sec"]
