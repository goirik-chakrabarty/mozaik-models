...
for aa_name in AA_list:
    datasets_mid[aa_name] = PickledDataStore(
        ...
        parameters=ParameterSet(
            {
                "root_directory": f"SelfSustainedPushPull_test:{aa_name}_____",
                "store_stimuli": False,
            }
        ),
        replace=False,
...
for ab_name in AB_list:
    datasets_mid[ab_name] = PickledDataStore(
        load=True,
        parameters=ParameterSet(
            {
                "root_directory": f"SelfSustainedPushPull_test:{ab_name}_____",
                "store_stimuli": False,
            }
        ),
        replace=False,
...
print("Pre-computing rates for odd segments (AA)...")
rates_odd_cache = {}
for name in AA_list:
    parsed = segment_cache_mid[name]
...
print("Pre-computing rates for A from AB segments (AB)...")
rates_even_cache = {}
for name in AB_list:
    parsed = segment_cache_mid[name]
    print(f"  Computing rates for {name} (A from AB segments)...")
    movie_A_name = parsed[0][1]["movie_name"]
    print(movie_A_name)
    A_from_AB = [
        seg for seg, stim_dict in parsed if stim_dict["movie_name"] == movie_A_name
    ]
    rates_even_cache[name] = calculate_rates(A_from_AB, window_duration_sec=1.0)

# Run comparisons using cached rates
for i, AA in enumerate(AA_list):
    for j, AB in enumerate(AB_list):
        rates_A = rates_odd_cache[AA]
        rates_A1 = rates_even_cache[AB]

        common_ids = sorted(set(rates_A.keys()) & set(rates_A1.keys()))
        print("Length of intersection of neuron ids:", len(common_ids))
        print(
            "Length of union - intersection of neuron ids:",
            len(set(rates_A.keys()) ^ set(rates_A1.keys())),
        )
        ...
        corr = plot_corr(
            values_A,
            values_A1,
            f"results/AB_time_comp_correct/response_comparison_{AA}_{AB}.png",
        )
        data[AB][AA] = corr
...
aa_labels = (
    AA_list  # ["AA1sec", "AA3sec", "AA5sec", "AA7sec", "AA9sec", "AA15sec", "AA25sec"]
)
ab_labels = [
    x for x in reversed(AB_list)
]  # ["AB1sec", "AB3sec", "AB5sec", "AB7sec", "AB9sec", "AB15sec", "AB25sec"]
