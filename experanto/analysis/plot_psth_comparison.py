...
# ── Configuration ──────────────────────────────────────────────────────
PSTH_DIR = "results/psth_100trials"  # where the .npy files from compute_psth.py are stored
...
def plot_comparison(aa_name, ab_name, neuron_ids, output_dir, n_cols=N_COLS):
    ...
    for idx in range(n_neurons):
        ...
        for label, psth_arr, sem_arr in traces:
            if psth_arr is not None:
                neuron_traces.append((label, psth_arr[idx], sem_arr[idx] if sem_arr is not None else None))
        
        _plot_psth_traces(ax, bin_centres, neuron_traces)

        ax.set_title(f"N{neuron_ids[idx]} (r={corr:.4f})", fontsize=7, pad=2) #  ± {corr_se:.4f}
    ...
    fig.suptitle(f"PSTH Comparison — {pair_label}  (bin={bins[1]:.0f} ms) - Mean: {np.mean(corr_list):.4f} +/- {np.std(corr_list):.4f}", fontsize=12)
...
def plot_population_comparison(aa_name, ab_name, neuron_ids, output_dir):
    ...
    def get_pop_stats(psth_arr):
        if psth_arr is None: return None, None
    ...
    ax.set_title(f"Population PSTH Comparison: {aa_name} / {ab_name}{corr_str}", fontsize=12)
    ax.set_xlabel("Time (ms)", fontsize=10)
    ax.set_ylabel("Mean Firing Rate (Hz)", fontsize=10)
    ax.legend(loc="upper right", fontsize=10)
    fig.tight_layout()

    save_path = os.path.join(output_dir, f"population_psth_comparison_{aa_name}_{ab_name}.png")
    fig.savefig(save_path, dpi=150)
    plt.close(fig)
    print(f"Saved: {save_path}")
...
def main():
    ...
    for aa_name, ab_name in CONDITION_PAIRS:
        ...
        mean, std = plot_comparison(aa_name, ab_name, neuron_ids, OUTPUT_DIR + "/AA0_vs_ABA")
    ...
    x_vals = [float(aa.split("_")[1][:-3]) for aa, _ in CONDITION_PAIRS]  # extract timing from AA name
    plt.errorbar(x_vals, mean_list, yerr=std_list, fmt='o', color='steelblue', ecolor='lightgray', capsize=5)
    ...
    plt.tight_layout()
    summary_path = os.path.join(OUTPUT_DIR, "AA0_vs_ABA", "correlation_summary_AA0_vs_ABA_test.png")
