...
def create_experiments_spont(model):
    return [
        # Spontaneous Activity
        NoStimulation(model, ParameterSet({"duration": 1001})),
    ]
...
def create_experiments_video(model):
    ...
    return [
        MeasurePixelMovieExperanto(
            model,
            ParameterSet(
                {
                    "base_path": "/data/dynamic29513-my-images-VA-1sec",
                    "movie_name": "",
                    "width": 11,
...
def create_randomized_experanto(model):
...
def create_experiments_spont_STW(model):
    return [
        # Spontaneous Activity
        NoStimulation(model, ParameterSet({"duration": 2 * 8 * 5 * 3 * 3 * 4 * 7})),
    ]
...
def create_experiments_spont_MSA(model):
    return [
        # Spontaneous Activity
        NoStimulation(model, ParameterSet({"duration": 120 * 1001})),
    ]


def patterned_opt_stim(
    model, images_path, intensity, num_trials=5, sheet="V1_Exc_L2/3"
):
    experiments = []
...
def create_experiments_optogenetic_fullfield_stimulation(model):
    return patterned_opt_stim(
        model, "optogenetic_stimulation_patterns/fullfield.npy", 0.033, 40
    )


def create_experiments_optogenetic_endogenous_stimulation(model):
    return patterned_opt_stim(
        model, "optogenetic_stimulation_patterns/endogenous", 0.033 * 0.76, 5
    )


def create_experiments_optogenetic_surrogate_stimulation(model):
    return patterned_opt_stim(
        model, "optogenetic_stimulation_patterns/surrogate", 0.033 * 0.76, 5
    )
...
def fullfield_gratings_8_orientations(model):
    return [
        # Spontaneous Activity
        NoStimulation(model, ParameterSet({"duration": 1001 * 5})),
        MeasureOrientationTuningFullfield(
            model,
            ParameterSet(
                {
